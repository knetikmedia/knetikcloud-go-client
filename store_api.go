...
/* StoreApiService Get a listing of store items
 The exact structure of each items may differ to include fields specific to the type. The same is true for behaviors.

 @param optional (nil or map[string]interface{}) with one or more of:
     @param "limit" (int32) The amount of items returned
     @param "page" (int32) The page of the request
     @param "useCatalog" (bool) Whether to remove items that are not intended for display or not in date
     @param "ignoreLocation" (bool) Whether to ignore country restrictions based on the caller&#39;s location
     @param "inStockOnly" (bool) Whether only in-stock items should be returned.  Default value is false
 @return PageResourceStoreItem*/
func (a *StoreApiService) GetStore(localVarOptionals map[string]interface{}) (PageResourceStoreItem,  *http.Response, error) {
	var (
		localVarHttpMethod = strings.ToUpper("Get")
		localVarPostBody interface{}
		localVarFileName string
	...
	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/store"
	...
	if err := typeCheckParameter(localVarOptionals["useCatalog"], "bool", "useCatalog"); err != nil {
		return successPayload, nil, err
	}
	if err := typeCheckParameter(localVarOptionals["ignoreLocation"], "bool", "ignoreLocation"); err != nil {
		return successPayload, nil, err
	}
	if err := typeCheckParameter(localVarOptionals["inStockOnly"], "bool", "inStockOnly"); err != nil {
		return successPayload, nil, err
	}
	...
	if localVarTempParam, localVarOk := localVarOptionals["useCatalog"].(bool); localVarOk {
		localVarQueryParams.Add("use_catalog", parameterToString(localVarTempParam, ""))
	}
	if localVarTempParam, localVarOk := localVarOptionals["ignoreLocation"].(bool); localVarOk {
		localVarQueryParams.Add("ignore_location", parameterToString(localVarTempParam, ""))
	}
	if localVarTempParam, localVarOk := localVarOptionals["inStockOnly"].(bool); localVarOk {
		localVarQueryParams.Add("in_stock_only", parameterToString(localVarTempParam, ""))
	}
	// to determine the Content-Type header
	localVarHttpContentTypes := []string{ "application/json",  }
